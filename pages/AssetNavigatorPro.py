...
with st.expander("📚 設定の目安と用語解説（ここをクリック）"):
    ...
    with col_h2:
        ...
        * **10.0%**: 10年に一度（必ず一度は経験する前提の嵐）
        """)
    st.info("※1σ（シグマ）は68%の確率、2σは95%の確率で収まる範囲です。")
...
with st.sidebar:
    ...
    if "Accumulation" in mode:
        ...
        investment_period = 0
        total_principal = initial_asset # 取崩は初期資産を元本とする
...
if run_button:
    ...
    # 指標計算：元本維持率は「投資した総額」を基準にする
    success_rate = (np.sum(final_p > 0.1) / sim_count) * 100
    ...
    m3.metric("元本維持率 (損なし)", f"{prob_above_principal:.1f} %")
    ...
    if use_bs:
        # 暴落が実際に起きたパスを優先的に1つ選んで表示
        bs_paths = [i for i, log in enumerate(bs_logs) if len(log) > 0]
        idx = np.random.choice(bs_paths) if bs_paths else np.random.randint(0, sim_count)
    ...
    stats = {
        ...
        'Principal (投資元本)': (total_principal, '#6f42c1', 'dash', 3),
    ...
    t_vals = [total_principal, total_principal * 1.5, total_principal * 2.0, 10000]
    t_names = ['元本維持以上', '元本の1.5倍', '元本の2.0倍', '資産1億円以上']
    t_probs = [(np.sum(final_p >= v) / sim_count) * 100 for v in t_vals]
    
    cols = st.columns(len(t_vals))
    ...
    st.info("条件を設定して、シミュレーションを実行してください。")